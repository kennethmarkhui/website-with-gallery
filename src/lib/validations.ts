...
export const GalleryFormFieldsSchema = z.object({
  ...
  image: z
    .custom<FileList>(
      // https://github.com/colinhacks/zod/issues/387#issuecomment-1774603011
      // bypass typechecking without ((v) => v instanceof FileList) for server side
      typeof window !== 'undefined' ? (v) => v instanceof FileList : undefined
    )
    .superRefine((fileList, ctx) => {
      ...
      if (isSingle && fileList[0].size >= MAX_FILE_SIZE) {
        ctx.addIssue({
          ...
          message: `Image must not exceed ${formatBytes(MAX_FILE_SIZE)}.`,
        })
      }
      if (isSingle && !ACCEPTED_IMAGE_TYPES.includes(fileList[0].type)) {
        ctx.addIssue({
          ...
          message: 'Image unsupported filetype.',
        })
      }
    })
    .optional()
